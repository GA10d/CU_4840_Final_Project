...
A
The software does not crash immediately. For rendering, it falls back from MMIO VGA to Linux framebuffer or console. For audio, the default context first tries the WM8731 MMIO path and can use command-line players like aplay, ffplay, or afplay when command audio is enabled. This makes debugging easier because gameplay can still run even if one hardware path is not ready.
