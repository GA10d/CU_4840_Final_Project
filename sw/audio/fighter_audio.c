...
enum {
  /*
   * The current FPGA serializer derives LRCK from the 50 MHz board clock
   * with a /1024 divider, so the effective playback rate is about 48.828 kHz.
   * We resample into that fixed-rate stream to keep the title BGM close to
   * the intended pitch on the current branch's hardware.
   */
  FIGHTER_AUDIO_MMIO_TARGET_RATE = 48828,
...
static const off_t k_fighter_audio_default_bridge_reset_addr =
    (off_t)0xFFD0501C;
...
static int fighter_audio_copy_string(const char *src, char *dst,
                                     size_t dst_size) {
...
static int fighter_audio_env_requests_command_backend(void) {
  const char *backend = getenv("FIGHTER_AUDIO_BACKEND");

  return backend && strcmp(backend, "command") == 0;
}
...
static void fighter_audio_build_once_command(
    const fighter_audio_context_t *context, const char *quoted_path,
...
static int fighter_audio_prepare_command_backend(
...
static int fighter_audio_resample_pcm16(fighter_audio_clip_t *clip,
  ...
  dst_frame_count = (size_t)(((uint64_t)src_frame_count *
  ...
  dst_samples = (int16_t *)malloc(dst_frame_count * 2U * sizeof(int16_t));
  if (!dst_samples) {
    return -1;
  }
  ...
  for (i = 0; i < dst_frame_count; ++i) {
    ...
    size_t src_index =
        (size_t)(src_position_num / FIGHTER_AUDIO_MMIO_TARGET_RATE);
    uint32_t frac =
...
static int fighter_audio_map_physical(int mem_fd, off_t physical_addr,
...
static void fighter_audio_mmio_unmap_region(void **map_base,
                                            size_t *map_length) {
...
static void fighter_audio_mmio_set_track_locked(
    fighter_audio_mmio_state_t *state, fighter_audio_track_t track,
    int loop_enabled) {
  if (!state) {
    return;
  }
...
static void fighter_audio_mmio_fill_fifo_locked(
...
static void fighter_audio_mmio_destroy(fighter_audio_mmio_state_t *state) {
  ...
  fighter_audio_mmio_unmap_region(&state->bridge_map,
...
static int fighter_audio_mmio_init(fighter_audio_context_t *context) {
  ...
  if (state->mem_fd < 0) {
    fighter_audio_set_status_detail(
        context, "WM8731 MMIO: open /dev/mem failed: %s", strerror(errno));
    fighter_audio_mmio_destroy(state);
    return -1;
  }
  ...
  if (fighter_audio_map_physical(state->mem_fd, bridge_reset_addr,
    ...
    fighter_audio_set_status_detail(
        context, "WM8731 MMIO: map bridge reset 0x%08lX failed: %s",
  ...
  if (fighter_audio_mmio_enable_bridges(state) != 0) {
    fighter_audio_set_status_detail(
        context, "WM8731 MMIO: failed to enable FPGA bridges");
    fighter_audio_mmio_destroy(state);
    return -1;
  }
  ...
  if (fighter_audio_map_physical(state->mem_fd, mmio_addr,
                                 ...
                                 &state->audio_regs) != 0) {
    fighter_audio_set_status_detail(
        context, "WM8731 MMIO: map audio core 0x%08lX failed: %s",
  ...
  thread_create_result =
      pthread_create(&state->thread, NULL, fighter_audio_mmio_thread_main,
                     state);
  if (thread_create_result != 0) {
    fighter_audio_set_status_detail(
        context, "WM8731 MMIO: audio thread start failed: %s",
...
static void fighter_audio_start_loop_command(fighter_audio_context_t *context,
  ...
  if (!path ||
      fighter_audio_shell_quote(path, quoted_path, sizeof(quoted_path)) != 0) {
    return;
  }
...
int fighter_audio_init(fighter_audio_context_t *context,
  ...
  enable_audio = options && options->enable_command_audio;
  force_command_backend =
      (options && options->force_command_backend) ||
      fighter_audio_env_requests_command_backend();
  if (!enable_audio) {
    return 0;
  }

  if (!force_command_backend && fighter_audio_mmio_init(context) == 0) {
    return 0;
  }
  ...
  if (fighter_audio_prepare_command_backend(context) == 0 &&
    ...
    if (force_command_backend) {
      fighter_audio_set_status_detail(
          context, "command backend requested but no aplay/ffplay/afplay found");
    } else if (context->status_detail[0] == '\0') {
      fighter_audio_set_status_detail(
...
void fighter_audio_close(fighter_audio_context_t *context) {
  ...
  if (context->backend == FIGHTER_AUDIO_BACKEND_MMIO && context->backend_data) {
    fighter_audio_mmio_destroy((fighter_audio_mmio_state_t *)context->backend_data);
...
void fighter_audio_process_commands(
    fighter_audio_context_t *context,
