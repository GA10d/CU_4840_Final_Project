...
typedef enum {
  ...
  FIGHTER_ANIM_HIT,
  FIGHTER_ANIM_KO,
  FIGHTER_ANIM_ATTACK_NORMAL,
  FIGHTER_ANIM_ATTACK_FIREBALL,
  FIGHTER_ANIM_ATTACK_DRAGON_PUNCH,
  FIGHTER_ANIM_ATTACK_JUMP,
  FIGHTER_ANIM_ATTACK_SWEEP,
  FIGHTER_ANIM_COUNT
} fighter_anim_kind_t;
...
static fighter_anim_kind_t fighter_renderer_select_anim_kind(
...
static int fighter_renderer_anim_frame_count(fighter_anim_kind_t anim) {
  switch (anim) {
    case FIGHTER_ANIM_IDLE:
      return 4;
    case FIGHTER_ANIM_WALK:
      return 5;
    case FIGHTER_ANIM_JUMP:
      return 4;
    case FIGHTER_ANIM_CROUCH:
      return 3;
    case FIGHTER_ANIM_GUARD:
      return 2;
    case FIGHTER_ANIM_HIT:
      return 8;
    case FIGHTER_ANIM_KO:
      return 8;
    case FIGHTER_ANIM_ATTACK_NORMAL:
      return 32;
    case FIGHTER_ANIM_ATTACK_FIREBALL:
      return 8;
    case FIGHTER_ANIM_ATTACK_DRAGON_PUNCH:
      return 8;
    case FIGHTER_ANIM_ATTACK_JUMP:
      return 10;
    case FIGHTER_ANIM_ATTACK_SWEEP:
      return 8;
    default:
      return 1;
  }
...
static const char *fighter_renderer_player_frame_path(
    int player_index,
    fighter_anim_kind_t anim,
    int frame_index) {
  static char path[256];
  const char *character_dir = (player_index == 0) ? "Ken" : "Ryu";
  const char *anim_name = "idle";
  int frame_count = fighter_renderer_anim_frame_count(anim);
  ...
  switch (anim) {
    case FIGHTER_ANIM_IDLE:                anim_name = "idle"; break;
    case FIGHTER_ANIM_WALK:                anim_name = "walk"; break;
    case FIGHTER_ANIM_JUMP:                anim_name = "jump"; break;
    case FIGHTER_ANIM_CROUCH:              anim_name = "crouch"; break;
    case FIGHTER_ANIM_GUARD:               anim_name = "guard"; break;
    case FIGHTER_ANIM_HIT:                 anim_name = "hit"; break;
    case FIGHTER_ANIM_KO:                  anim_name = "ko"; break;
    case FIGHTER_ANIM_ATTACK_NORMAL:       anim_name = "attack_normal"; break;
    case FIGHTER_ANIM_ATTACK_FIREBALL:     anim_name = "attack_fireball"; break;
    case FIGHTER_ANIM_ATTACK_DRAGON_PUNCH: anim_name = "attack_dragon_punch"; break;
    case FIGHTER_ANIM_ATTACK_JUMP:         anim_name = "attack_jump"; break;
    case FIGHTER_ANIM_ATTACK_SWEEP:        anim_name = "attack_sweep"; break;
    default:                               anim_name = "idle"; break;
  }

  snprintf(path, sizeof(path),
           "../game_assets/sprites/%s/%s_%d.ppm",
           character_dir, anim_name, frame_index);
  return path;
}

static void fighter_fb_draw_rgb_image_at(
  ...
  if (!renderer || !image || !image->pixels || dst_w <= 0 || dst_h <= 0) {
    return;
  }

  for (y = 0; y < dst_h; ++y) {
    int src_y = (int)(((long long)y * image->height) / dst_h);
    const unsigned char *src_row =
        image->pixels + (size_t)src_y * (size_t)image->width * 3U;

    for (x = 0; x < dst_w; ++x) {
      int src_x = (int)(((long long)x * image->width) / dst_w);
      const unsigned char *src_pixel;

      if (flip_x) {
        src_x = image->width - 1 - src_x;
      }
      ...
      fighter_fb_put_pixel(renderer, dst_x + x, dst_y + y,
                           fighter_fb_color(renderer,
                                            src_pixel[0],
                                            src_pixel[1],
                                            src_pixel[2]));
    }
...
static fighter_anim_desc_t fighter_renderer_get_anim_desc(
    const fighter_player_state_t *player) {
  fighter_anim_desc_t desc;
...
static int fighter_renderer_get_anim_frame(const fighter_game_t *game,
                                           const fighter_player_state_t *player) {
  fighter_anim_desc_t desc;
  int progress;
  int frame;
  ...
  desc = fighter_renderer_get_anim_desc(player);

  switch (player->visual_state) {
    case FIGHTER_VISUAL_STATE_IDLE:
    case FIGHTER_VISUAL_STATE_WALK:
    case FIGHTER_VISUAL_STATE_GUARD:
      if (desc.frame_count <= 1 || desc.frame_hold <= 0) {
        return 0;
      }
      return (int)((game->frame_counter / (uint32_t)desc.frame_hold) %
                   (uint32_t)desc.frame_count);

    case FIGHTER_VISUAL_STATE_JUMP:
      return player->vy < 0 ? 0 : 1;

    case FIGHTER_VISUAL_STATE_ATTACK:
      if (game->config.attack_visual_frames <= 0) {
        return 0;
      }
      progress = game->config.attack_visual_frames - player->attack_visual_frames;
      if (progress < 0) {
        progress = 0;
      }
      frame = progress / (desc.frame_hold > 0 ? desc.frame_hold : 1);
      if (frame >= desc.frame_count) {
        frame = desc.frame_count - 1;
      }
      if (frame < 0) {
        frame = 0;
      }
    ...
    case FIGHTER_VISUAL_STATE_HIT:
      if (game->config.hurt_visual_frames <= 0) {
        return 0;
      }
      progress = game->config.hurt_visual_frames - player->hurt_visual_frames;
      if (progress < 0) {
        progress = 0;
      }
      frame = progress / (desc.frame_hold > 0 ? desc.frame_hold : 1);
...
static void fighter_renderer_draw_attack_effect_fb(
    fighter_renderer_t *renderer,
  ...
  int effect_w;
  int effect_h;
  unsigned int effect_color;

  if (!renderer || !game || !player || player->attack_visual_frames <= 0) {
    return;
  }

  effect_w = fighter_scale_size_axis(14, renderer->fb_width, 640);
  effect_h = fighter_scale_size_axis(14, renderer->fb_height, 480);
  effect_y = player_y + player_h / 3;

  switch (player->last_attack) {
    case FIGHTER_ATTACK_FIREBALL:
      effect_w = fighter_scale_size_axis(22 + anim_frame * 4, renderer->fb_width, 640);
      effect_h = fighter_scale_size_axis(22 + anim_frame * 4, renderer->fb_height, 480);
      effect_x = player->facing > 0 ? player_x + player_w + effect_w / 4
                                    : player_x - effect_w - effect_w / 4;
      effect_y = player_y + player_h / 2 - effect_h / 2;
      effect_color = fighter_fb_color(renderer, 255, 180, 60);
      break;

    case FIGHTER_ATTACK_DRAGON_PUNCH:
      effect_w = fighter_scale_size_axis(16, renderer->fb_width, 640);
      effect_h = fighter_scale_size_axis(20 + anim_frame * 10, renderer->fb_height, 480);
      effect_x = player->facing > 0 ? player_x + player_w - effect_w / 2
                                    : player_x - effect_w / 2;
      effect_y = player_y - effect_h / 3;
      effect_color = fighter_fb_color(renderer, 255, 240, 120);
      break;

    case FIGHTER_ATTACK_SWEEP:
      effect_w = fighter_scale_size_axis(26 + anim_frame * 6, renderer->fb_width, 640);
      effect_h = fighter_scale_size_axis(10, renderer->fb_height, 480);
      effect_x = player->facing > 0 ? player_x + player_w
                                    : player_x - effect_w;
      effect_y = player_y + player_h - effect_h -
                 fighter_scale_size_axis(6, renderer->fb_height, 480);
      effect_color = fighter_fb_color(renderer, 255, 220, 110);
      break;

    case FIGHTER_ATTACK_JUMP_ATTACK:
    case FIGHTER_ATTACK_FORWARD_JUMP_ATTACK:
    case FIGHTER_ATTACK_BACK_JUMP_ATTACK:
      effect_w = fighter_scale_size_axis(18, renderer->fb_width, 640);
      effect_h = fighter_scale_size_axis(18, renderer->fb_height, 480);
      effect_x = player->facing > 0 ? player_x + player_w
                                    : player_x - effect_w;
      effect_y = player_y + player_h / 3;
      effect_color = fighter_fb_color(renderer, 255, 245, 180);
      break;

    case FIGHTER_ATTACK_NORMAL:
    case FIGHTER_ATTACK_NONE:
    default:
      effect_w = fighter_scale_size_axis(12 + anim_frame * 4, renderer->fb_width, 640);
      effect_h = fighter_scale_size_axis(18, renderer->fb_height, 480);
      effect_x = player->facing > 0 ? player_x + player_w
                                    : player_x - effect_w;
      effect_color = fighter_fb_color(renderer, 255, 240, 120);
  ...
  fighter_fb_fill_rect(renderer, effect_x, effect_y, effect_w, effect_h, effect_color);
}

static void fighter_renderer_draw_player_fallback_fb(
    fighter_renderer_t *renderer,
    const fighter_game_t *game,
    const fighter_player_state_t *player,
  ...
  frame = fighter_renderer_get_anim_frame(game, player);
  dx = player->facing > 0 ? 1 : -1;

  body_w = fighter_scale_size_axis(24, renderer->fb_width, 640);
  body_h = fighter_scale_size_axis(40, renderer->fb_height, 480);
  head_w = fighter_scale_size_axis(20, renderer->fb_width, 640);
  head_h = fighter_scale_size_axis(18, renderer->fb_height, 480);
  arm_w = fighter_scale_size_axis(8, renderer->fb_width, 640);
  arm_h = fighter_scale_size_axis(18, renderer->fb_height, 480);
  leg_w = fighter_scale_size_axis(8, renderer->fb_width, 640);
  leg_h = fighter_scale_size_axis(22, renderer->fb_height, 480);

  body_x = player_x + (player_w - body_w) / 2;
  body_y = player_y + fighter_scale_size_axis(24, renderer->fb_height, 480);
  head_x = player_x + (player_w - head_w) / 2;
  head_y = player_y + fighter_scale_size_axis(4, renderer->fb_height, 480);

  arm_y = body_y + fighter_scale_size_axis(4, renderer->fb_height, 480);
  leg_y = body_y + body_h - fighter_scale_size_axis(2, renderer->fb_height, 480);

  rear_arm_x = body_x + (dx > 0 ? -arm_w + 1 : body_w - 1);
  lead_arm_x = body_x + (dx > 0 ? body_w - 1 : -arm_w + 1);

  back_leg_x = body_x + fighter_scale_size_axis(4, renderer->fb_width, 640);
  front_leg_x =
      body_x + body_w - leg_w - fighter_scale_size_axis(4, renderer->fb_width, 640);

  switch (player->visual_state) {
    case FIGHTER_VISUAL_STATE_IDLE:
      body_y += (frame & 1) ? 0 : fighter_scale_size_axis(1, renderer->fb_height, 480);
      head_y += (frame & 1) ? 0 : fighter_scale_size_axis(1, renderer->fb_height, 480);
      break;

    case FIGHTER_VISUAL_STATE_WALK:
      if ((frame % 2) == 0) {
        front_leg_x += fighter_scale_size_axis(3, renderer->fb_width, 640);
        back_leg_x -= fighter_scale_size_axis(2, renderer->fb_width, 640);
      } else {
        front_leg_x -= fighter_scale_size_axis(2, renderer->fb_width, 640);
        back_leg_x += fighter_scale_size_axis(3, renderer->fb_width, 640);
      }
      if ((frame % 3) == 0) {
        lead_arm_x += dx * fighter_scale_size_axis(3, renderer->fb_width, 640);
      } else {
        rear_arm_x -= dx * fighter_scale_size_axis(2, renderer->fb_width, 640);
      }
      break;

    case FIGHTER_VISUAL_STATE_JUMP:
      body_y -= fighter_scale_size_axis(frame == 0 ? 8 : 4, renderer->fb_height, 480);
      head_y -= fighter_scale_size_axis(frame == 0 ? 8 : 4, renderer->fb_height, 480);
      leg_h = fighter_scale_size_axis(frame == 0 ? 16 : 20, renderer->fb_height, 480);
      break;

    case FIGHTER_VISUAL_STATE_CROUCH:
      body_h = fighter_scale_size_axis(26, renderer->fb_height, 480);
      head_y += fighter_scale_size_axis(10, renderer->fb_height, 480);
      body_y += fighter_scale_size_axis(18, renderer->fb_height, 480);
      leg_h = fighter_scale_size_axis(10, renderer->fb_height, 480);
      leg_y = body_y + body_h - fighter_scale_size_axis(1, renderer->fb_height, 480);
      break;

    case FIGHTER_VISUAL_STATE_GUARD:
      lead_arm_x += dx * fighter_scale_size_axis(5 + frame, renderer->fb_width, 640);
      arm_y -= fighter_scale_size_axis(3, renderer->fb_height, 480);
      break;

    case FIGHTER_VISUAL_STATE_HIT:
      body_x -= dx * fighter_scale_size_axis(4, renderer->fb_width, 640);
      head_x -= dx * fighter_scale_size_axis(5, renderer->fb_width, 640);
      break;

    case FIGHTER_VISUAL_STATE_ATTACK:
      switch (player->last_attack) {
        case FIGHTER_ATTACK_FIREBALL:
          lead_arm_x += dx * fighter_scale_size_axis(10 + frame * 4, renderer->fb_width, 640);
          rear_arm_x += dx * fighter_scale_size_axis(2, renderer->fb_width, 640);
          break;
        case FIGHTER_ATTACK_DRAGON_PUNCH:
          body_y -= fighter_scale_size_axis(6 + frame * 2, renderer->fb_height, 480);
          head_y -= fighter_scale_size_axis(6 + frame * 2, renderer->fb_height, 480);
          lead_arm_x += dx * fighter_scale_size_axis(6, renderer->fb_width, 640);
          arm_y -= fighter_scale_size_axis(10 + frame * 3, renderer->fb_height, 480);
          break;
        case FIGHTER_ATTACK_SWEEP:
          body_h = fighter_scale_size_axis(22, renderer->fb_height, 480);
          body_y += fighter_scale_size_axis(18, renderer->fb_height, 480);
          head_y += fighter_scale_size_axis(10, renderer->fb_height, 480);
          front_leg_x += dx * fighter_scale_size_axis(8 + frame * 4, renderer->fb_width, 640);
          leg_h = fighter_scale_size_axis(10, renderer->fb_height, 480);
          leg_y = body_y + body_h - fighter_scale_size_axis(1, renderer->fb_height, 480);
          break;
        case FIGHTER_ATTACK_JUMP_ATTACK:
        case FIGHTER_ATTACK_FORWARD_JUMP_ATTACK:
        case FIGHTER_ATTACK_BACK_JUMP_ATTACK:
          body_y -= fighter_scale_size_axis(6, renderer->fb_height, 480);
          head_y -= fighter_scale_size_axis(6, renderer->fb_height, 480);
          front_leg_x += dx * fighter_scale_size_axis(6, renderer->fb_width, 640);
          lead_arm_x += dx * fighter_scale_size_axis(8 + frame * 2, renderer->fb_width, 640);
          break;
        case FIGHTER_ATTACK_NORMAL:
        case FIGHTER_ATTACK_NONE:
        default:
          lead_arm_x += dx * fighter_scale_size_axis(8 + frame * 4, renderer->fb_width, 640);
          break;
      }
      break;
    ...
    case FIGHTER_VISUAL_STATE_KO:
      fighter_fb_fill_rect(
          renderer,
          player_x + fighter_scale_size_axis(6, renderer->fb_width, 640),
          player_y + player_h - fighter_scale_size_axis(16, renderer->fb_height, 480),
          fighter_scale_size_axis(36, renderer->fb_width, 640),
          fighter_scale_size_axis(12, renderer->fb_height, 480), body_color);
      fighter_fb_fill_rect(
          renderer,
          player_x + fighter_scale_size_axis(40, renderer->fb_width, 640),
          player_y + player_h - fighter_scale_size_axis(18, renderer->fb_height, 480),
          fighter_scale_size_axis(12, renderer->fb_width, 640),
          fighter_scale_size_axis(10, renderer->fb_height, 480), body_color);
      eye_size = fighter_scale_size_axis(4, renderer->fb_width, 640);
      fighter_fb_fill_rect(
          renderer,
          player_x + fighter_scale_size_axis(46, renderer->fb_width, 640),
          player_y + player_h - fighter_scale_size_axis(15, renderer->fb_height, 480),
          eye_size, eye_size, fighter_fb_color(renderer, 250, 250, 250));
      return;
  ...
  fighter_fb_fill_rect(renderer, body_x, body_y, body_w, body_h, body_color);
  fighter_fb_fill_rect(renderer, head_x, head_y, head_w, head_h, body_color);
  fighter_fb_fill_rect(renderer, rear_arm_x, arm_y, arm_w, arm_h, body_color);
  fighter_fb_fill_rect(renderer, lead_arm_x, arm_y, arm_w, arm_h, body_color);
  fighter_fb_fill_rect(renderer, back_leg_x, leg_y, leg_w, leg_h, body_color);
  fighter_fb_fill_rect(renderer, front_leg_x, leg_y, leg_w, leg_h, body_color);

  eye_size = fighter_scale_size_axis(4, renderer->fb_width, 640);
  eye_x = player->facing > 0
              ? head_x + head_w - fighter_scale_size_axis(6, renderer->fb_width, 640)
              : head_x + fighter_scale_size_axis(2, renderer->fb_width, 640);
  eye_y = head_y + fighter_scale_size_axis(5, renderer->fb_height, 480);
  fighter_fb_fill_rect(renderer, eye_x, eye_y, eye_size, eye_size,
                       fighter_fb_color(renderer, 250, 250, 250));

  if (player->visual_state == FIGHTER_VISUAL_STATE_HIT) {
    fighter_fb_fill_rect(
        renderer,
        head_x - dx * fighter_scale_size_axis(4, renderer->fb_width, 640),
        head_y + fighter_scale_size_axis(2, renderer->fb_height, 480),
        fighter_scale_size_axis(4, renderer->fb_width, 640),
        fighter_scale_size_axis(4, renderer->fb_height, 480),
        fighter_fb_color(renderer, 255, 255, 120));
  }

  fighter_renderer_draw_attack_effect_fb(renderer, game, player,
                                         player_x, player_y, player_w, player_h, frame);
}
...
static void fighter_renderer_draw_player_fb(
  ...
  anim = fighter_renderer_select_anim_kind(player);
  frame = fighter_renderer_get_anim_frame(game, player);
  path = fighter_renderer_player_frame_path(player == &game->players[0] ? 0 : 1,
                                            anim, frame);

  if (fighter_rgb_image_load_ppm(&sprite, path) == 0 && sprite.pixels) {
    fighter_fb_draw_rgb_image_at(renderer, &sprite,
                                 player_x, player_y,
                                 player_w, player_h,
                                 player->facing < 0);
    fighter_rgb_image_reset(&sprite);
    fighter_renderer_draw_attack_effect_fb(renderer, game, player,
                                           player_x, player_y, player_w, player_h, frame);
    return;
  }

  fighter_renderer_draw_player_fallback_fb(renderer, game, player, body_color,
...
static void fighter_renderer_draw_playfield_fb(fighter_renderer_t *renderer,
  ...
  fighter_fb_fill_rect(renderer, 0, 0, renderer->fb_width, floor_y, sky_top);
  fighter_fb_fill_rect(renderer, 0, floor_y, renderer->fb_width, floor_height, floor_color);
  ...
  fighter_fb_draw_text(
      renderer,
      renderer->fb_width - fighter_scale_axis(60, renderer->fb_width, 640),
      label_y, "P2", text_scale_small, text_color);
  ...
  for (i = 0; i < FIGHTER_PLAYER_COUNT; ++i) {
    ...
    player_y = fighter_scale_axis(player->y, renderer->fb_height, game->config.screen_height);
    ...
    fighter_renderer_draw_player_fb(renderer, game, player, player_color,
                                    player_x, player_y, player_w, player_h);
  }
...
static void fighter_renderer_draw_console(fighter_renderer_t *renderer,
  ...
  switch (game->state) {
    ...
    case FIGHTER_GAME_STATE_PLAYING:
      printf("[frame %u] PLAY timer=%d "
             "P1(x=%d y=%d hp=%d state=%d atk=%d frm=%d asset=%s) "
             "P2(x=%d y=%d hp=%d state=%d atk=%d frm=%d asset=%s)\n",
             game->frame_counter, fighter_game_round_seconds_remaining(game),
             game->players[0].x, game->players[0].y, game->players[0].hp,
             game->players[0].visual_state, game->players[0].last_attack,
             fighter_renderer_get_anim_frame(game, &game->players[0]),
             fighter_renderer_player_frame_path(
                 0, fighter_renderer_select_anim_kind(&game->players[0]),
                 fighter_renderer_get_anim_frame(game, &game->players[0])),
             game->players[1].x, game->players[1].y, game->players[1].hp,
             game->players[1].visual_state, game->players[1].last_attack,
             fighter_renderer_get_anim_frame(game, &game->players[1]),
             fighter_renderer_player_frame_path(
                 1, fighter_renderer_select_anim_kind(&game->players[1]),
                 fighter_renderer_get_anim_frame(game, &game->players[1])));
      break;
