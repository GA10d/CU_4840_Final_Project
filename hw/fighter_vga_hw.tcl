...
#
# 中文说明：
# 这个 Tcl 文件把 fighter_vga_renderer.sv 注册成 Platform Designer/Qsys IP。
# 关键接口是 avalon_slave_0：
# - addressUnits WORDS 表示地址按 32-bit word 计数，和软件 regs[offset] 一致。
# - avs_address 宽度 16 bit，可覆盖 65536 个 word；framebuffer 需要
#   1024 + 38400 个 word，所以 16 bit 足够，15 bit 不够。
# - avs_writedata/avs_readdata 宽度 32 bit，对应 Avalon-MM 数据总线，也让
#   一个 word 正好打包两个 16-bit RGB565 像素。
# VGA conduit 中 R/G/B 各 8 bit，匹配开发板 VGA DAC 的颜色输入位宽。
...
# 16-bit word address：寻址控制寄存器和 320x240/2 个 framebuffer word。
add_interface_port avalon_slave_0 avs_address address Input 16
# 32-bit data：Avalon-MM word 宽度；framebuffer 每 word 两个 RGB565 像素。
